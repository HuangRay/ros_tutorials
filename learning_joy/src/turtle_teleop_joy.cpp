...
//setsp = (X^2+Y^2)^(1/2) ->maxinum=1
  ...
  double ls_up=0,ls_r_left=0,ls_r_right=0,ls_down=0,ls_grip_close=0,ls_grip_open=0;
  /*set grip*/
  ...
  /*initial*/
  if(ls_up==1){up=0;}
  if(ls_r_left==1){r_left=0;}
  if(ls_r_right==1){r_right=0;}
  if(ls_down==1){down=0;}
  if(ls_grip_close==1){grip_close=0;}
  if(ls_grip_open==1){grip_open=0;}
  /*publish*/  
  if(up==1){                    //button Y ==1 -> up ->publish(w)
    arm.data='w';
    arm_pub_.publish(arm);
    ls_up=up;
  }else if(down==1){		//button A == 1 -> down ->->publish(s)
    arm.data='s';
    arm_pub_.publish(arm);
    ls_down=down;
  }else if(r_left==1){      	//button X ==1 ->rotate left ->->publish(a)
    arm.data='a';
    arm_pub_.publish(arm);
    ls_r_left=r_left;
  }else if(r_right==1){    	//button B ==1 ->rotate right ->->publish(d)
    arm.data='d';
    arm_pub_.publish(arm);
    ls_r_right=r_right;
  }else if(grip_close==1){	//LT !=1 ->grip close ->->publish(o)
    arm.data='o';
    arm_pub_.publish(arm);
    ls_grip_close=grip_close;
  }else if(grip_open==1){	//RT !=1 ->grip open ->->publish(p)
    arm.data='p';
    arm_pub_.publish(arm);
